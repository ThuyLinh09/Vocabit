...
public class ImageToTextQuestionService {
    ImageToTextQuestionRepository repository;
    ImageToTextQuestionMapper mapper;

    public List<ImageToTextQuestionResponse> getRandomQuestionsByUnit(int unit) {
        var questions = repository.findRandom10ByUnit(unit);

        // Trộn đáp án mỗi câu
        questions.forEach(q -> Collections.shuffle(q.getOptions()));

        return mapper.toImageToTextQuestionList(questions);
    }
}
